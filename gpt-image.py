...
# 스크립트와 같은 디렉터리의 .env 로드함
load_dotenv(Path(__file__).resolve().parent / ".env")

# .env 또는 OS 환경 변수에서 읽음
endpoint = os.getenv("AZURE_OPENAI_ENDPOINT")
api_version = os.getenv("OPENAI_API_VERSION")
deployment = os.getenv("DEPLOYMENT_NAME")
api_key = os.getenv("AZURE_OPENAI_API_KEY")
if not api_key:
    raise SystemExit("AZURE_OPENAI_API_KEY가 비어 있음. .env 또는 환경 변수에 설정함.")
...
def _generate_with_progress(prompt_text: str, size: str):
...
# prompt_toolkit으로 한글 입력·백스페이스 처리함
...
_ratio_choice = prompt("번호 선택 (Enter=1): ").strip()
if not _ratio_choice:
    _ratio_choice = "1"
if _ratio_choice not in _ratio_map:
    raise SystemExit("1~4 중 번호를 입력하거나 Enter만 누름.")
image_size = _ratio_map[_ratio_choice]

t_start = time.perf_counter()
result = _generate_with_progress(user_prompt, image_size)
